...
const pathUser = "http://localhost:8080/users"

async function filterUser() {
    let searchWord = document.getElementById('findUser').value
    const param = new URLSearchParams({
        "str": searchWord
    })
    fetch(pathUser + "/filter?" + param).then(response => response.text()).then(fragment => {
        document.querySelector(".user_list").innerHTML = fragment
        eventForUserPage();
    })
